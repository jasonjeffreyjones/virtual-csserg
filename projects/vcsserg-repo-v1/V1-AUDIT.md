...
| Documented promise | Evidence or test | Status on September 21, 2026 |
|---|---|---|
...
| Deployment safely mirrors `website/` | Mocked guarded transfer plus checksum/inventory dry run; completed normal workflow; public expected-file probe | Pass; the September 15 run reached completion only after exact-inventory verification, all 110 expected files matched production on September 16, 114 matched before the September 18 revision, 128 before the September 19 revision, 134 before the September 20 revision, and 142 before the September 21 revision |

## Current automated result

On September 21, 2026, **all seven groups pass**. The final report-format gap
...
- requires every page's first anchor itself to target `main` as a bypass and
  every image to carry an explicit `alt` attribute, with negative fixtures for
  missing, late, wrong-target, misleading-first, and runtime-relocated bypasses
  plus a missing image alternative;
...
revision, all 142 incoming files were byte-identical; normal automation must
still deploy and inventory the revised release.
