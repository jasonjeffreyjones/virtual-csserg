...
Bee Boring Vanilla · Virtual CSSERG · September 26, 2026
...
Repository guidance passes: the governing files, growth, migration, report-archive procedures, and tested creation commands exist. Project memory passes: every Project and the template have PROJECT.md, STATE.md, a bounded DIALOG.md, immutable iteration/year indexes, verified legacy hashes where applicable, and independent lifecycle/publication metadata. Static HTML/CSS passes structural, first-anchor bypass-link, named-navigation, named-data-table, scoped-table-header, explicit-image-alternative, interactive-name/ARIA, branding, local-link, Bootstrap, and footer checks. A tested post-render normalizer makes the bypass link physically first, names repeated navigation, and gives generated table-head cells explicit column scope instead of relying on runtime JavaScript or hand edits. All 26 public data tables now have a source-owned caption, including 24 added in this iteration; the whole-site parser rejects unnamed tables and broken or empty table labels. It also rejects unnamed exposed interactive or keyboard-focusable elements, broken control references, invalid expanded states, and focusable controls hidden from assistive technology. Coverage includes links, buttons, native form and disclosure controls, interactive ARIA roles, media controls, and custom focus targets. Public catalogs pass Published-Project and rostered-Scholar coverage, canonical-biography fidelity, lifecycle status, and metadata-derived update order. Runner guards and exact-mirror deployment, including its post-transfer checksum/inventory dry run, pass.
