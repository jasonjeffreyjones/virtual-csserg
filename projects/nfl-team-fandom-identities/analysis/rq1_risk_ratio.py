...
def normalize_yes_no(value: str, source: Path, row_number: int, column: str) -> bool:
...
def load_records(
    ...
    demographic_keys, demographic_rows = read_demographic_keys(
        demographics_path, id_column, date_column
    )
    ...
    with responses_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            if previous is not None:
                if previous != answer:
                    raise AnalysisError(
                        f"{responses_path} has conflicting answers for {signifier!r} "
                        f"at respondent-day {key!r}"
                    )
    ...
    for (respondent_id, obs_date), answers in answers_by_key.items():
        if fandom_signifier in answers and outcome_signifier in answers:
            records.append(
                (
                    respondent_id,
                    obs_date,
    ...
    audit: Dict[str, object] = {
        ...
        "unique_demographic_join_keys": len(demographic_keys),
...
def bootstrap_clustered(
    ...
    for _ in range(reps):
        ...
        rd_values.append(exposed_prevalence - reference_prevalence)
        if reference_prevalence > 0:
            rr_values.append(exposed_prevalence / reference_prevalence)
    ...
    return {
        ...
        "risk_ratio_successful_replicates": len(rr_values),
        "risk_difference_successful_replicates": len(rd_values),
        "seed": seed,
        "ci_level": ci_level,
        "risk_ratio_ci": (
            [
                quantile(rr_values, lower_probability),
                quantile(rr_values, upper_probability),
            ]
            if rr_values
            else None
        ),
        "risk_difference_ci": (
            [
                quantile(rd_values, lower_probability),
                quantile(rd_values, upper_probability),
            ]
            if rd_values
            else None
        ),
...
def estimate(records: Sequence[Record], ci_level: float, bootstrap_reps: int, seed: int) -> Dict[str, object]:
    ...
    risk_difference = exposed_prevalence - reference_prevalence
    z = NormalDist().inv_cdf(0.5 + ci_level / 2)

    risk_ratio_ci = None
    if a > 0 and c > 0 and math.isfinite(risk_ratio):
        log_standard_error = math.sqrt(
            (1 / a) - (1 / exposed_n) + (1 / c) - (1 / reference_n)
        )
        risk_ratio_ci = [
            math.exp(math.log(risk_ratio) - z * log_standard_error),
            math.exp(math.log(risk_ratio) + z * log_standard_error),
        ]
    ...
    risk_difference_ci = [
        risk_difference - z * difference_standard_error,
        risk_difference + z * difference_standard_error,
    ]
    ...
    return {
        ...
        "risk_ratio": risk_ratio if math.isfinite(risk_ratio) else None,
        "risk_ratio_ci": risk_ratio_ci,
        "risk_ratio_ci_method": "Katz log interval (respondent-days treated as independent)",
        "risk_difference": risk_difference,
        "risk_difference_ci": risk_difference_ci,
        "risk_difference_ci_method": "unpooled Wald interval (respondent-days treated as independent)",
        "ci_level": ci_level,
...
def print_summary(result: Mapping[str, object]) -> None:
    ...
    rr = estimates["risk_ratio"]
    rr_text = "undefined" if rr is None else f"{float(rr):.4f}"
    ...
    print(f"Risk (prevalence) ratio: {rr_text}")
    print(f"Risk difference: {100 * float(estimates['risk_difference']):+.2f} percentage points")
